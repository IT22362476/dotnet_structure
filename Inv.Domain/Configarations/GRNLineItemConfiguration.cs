...
namespace Inv.Domain.Configarations
{
    public class GRNLineItemConfiguration : IEntityTypeConfiguration<GRNLineItem>
    {
        ...
        {
            builder.HasKey(c => c.GRNLineItemSerialID);

            builder.Property(e => e.GRNLineItemID).HasDefaultValueSql("SELECT NEXT VALUE FOR dbo.GRNLineItemID");

            builder.Property(c => c.GRNLineItemID).HasColumnName("GRNLineItemID").IsRequired();

            builder.HasIndex(c => c.GRNLineItemID).IsUnique();
