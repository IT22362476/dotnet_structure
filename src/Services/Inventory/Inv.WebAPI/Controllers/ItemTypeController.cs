...
namespace Inv.WebAPI.Controllers
{
    ...
    public class ItemTypeController : GenericController<ItemType, ApplicationDbContext, GetItemTypeDto, CreateItemTypeDto, UpdateItemTypeDto>
    {
        ...
        public ItemTypeController(ApplicationDbContext context, ILogger<ItemTypeController> logger, IMediator mediator, IMapper mapper, TheNumbersService theNumbersService)
: base(context, mapper, theNumbersService)
        ...
        [HttpGet(Name = "GetItemType")]
        [AuthorizeMultiplePermissions(Permissions.Btn_SetViewItemType)]
        public override async Task<ActionResult<IEnumerable<GetItemTypeDto>>> GetAll()
        {
        ...
        [HttpGet("{id}")]
        [AuthorizeMultiplePermissions(Permissions.Btn_SetViewItemType)]
        public override async Task<ActionResult<GetItemTypeDto>> Get(int id)
        {
        ...
        [HttpPost(Name = "CreateGetItemType")]
        [AuthorizeMultiplePermissions(Permissions.Btn_SetSaveItemType)]
        public override async Task<ActionResult<GetItemTypeDto>> Create([FromBody] CreateItemTypeDto createDto,[FromQuery] string[]? uniquePropertyNames = null)
        {
            return await base.Create(createDto, new[] { "ItemTypeName" });
        }
        ...
        ///  <param name="uniquePropertyNames">Optional: Array of property names to check for uniqueness.</param>
        [HttpPut]
        [AuthorizeMultiplePermissions(Permissions.Btn_SetEditItemType)]
        public override async Task<ActionResult<GetItemTypeDto>> Update([FromBody] UpdateItemTypeDto updateDto, [FromQuery] string[]? uniquePropertyNames = null)
        {
            return await base.Update(updateDto, new[] { "ItemTypeName" });
        }
        ...
        [HttpDelete("{id}")]
        [AuthorizeMultiplePermissions(Permissions.Btn_SetDeleteItemType)]
        ...
        [AuthorizeMultiplePermissions(Permissions.Btn_SetViewItemType)]
        public async Task<ActionResult<IEnumerable<GetItemTypeDto>>> GetByCustomCriteria([FromQuery] string criteria)
        {
            // Example of custom filtering logic
            var entities = await _context.Set<ItemType>()
                .Where(a => a.ItemTypeName.Contains(criteria)) // Filter by name containing the criteria
                .ToListAsync();

            var dtos = _mapper.Map<List<GetItemTypeDto>>(entities);
            return Ok(dtos);
        }
