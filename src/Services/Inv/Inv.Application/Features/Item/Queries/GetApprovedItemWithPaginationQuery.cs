...
namespace Inv.Application.Features.Item.Queries
{
    ...
    public record GetApprovedItemWithPaginationQuery : IRequest<Result<PaginatedResult<GetItemsWithPaginationDto>>>
    {
        ...
        public GetApprovedItemWithPaginationQuery() { }

        public GetApprovedItemWithPaginationQuery(int pageNumber, int pageSize)
        {
    ...
    internal class GetApprovedItemsWithPaginationQueryHandler : IRequestHandler<GetApprovedItemWithPaginationQuery, Result<PaginatedResult<GetItemsWithPaginationDto>>>
    {
        ...
        public GetApprovedItemsWithPaginationQueryHandler(IUnitOfWork unitOfWork, IMapper mapper, ISqlDataAccess sqlDataAccess, ISortHelper<GetItemsWithPaginationDto> sortHelper)
        {
        ...
        public async Task<Result<PaginatedResult<GetItemsWithPaginationDto>>> Handle(GetApprovedItemWithPaginationQuery query, CancellationToken cancellationToken)
        {
            // Correct way
            GetApprovedItemsWithPaginationValidator validator = new GetApprovedItemsWithPaginationValidator();
            ...
            if (validationResult.IsValid)
            {
                ...
                var sql = @"SELECT ROW_NUMBER() OVER(ORDER BY itm.ItemSerialID) AS Id,itm.ItemSerialID as ItemSerialID,[ItemID] ,[ItemCode],itm.[ItemTypeSerialID],itm.[ApprovedBy],itm.[ApprovedDate],usr.UserName as Approved,usrm.UserName as Modified,ityp.ItemTypeName,b.BrandName+' '+m.ModelName+' '+c.MainCategoryName+' '+sc.SubCategoryName as [ItemDes],itm.[MainCategorySerialID],c.MainCategoryName,itm.[SubCategorySerialID],sc.SubCategoryName,itm.[ModelSerialID],m.ModelName,itm.[BrandSerialID],b.BrandName,[Weight],[Volume],[Size],[Color],[ItemPartNo],[Article],[Remarks],[Length],[Width],[Height],[Guage],[Construction],[SpecialFeatures],itm.[UOMSerialID],u.UOMName,itm.[CreatedBy],itm.[CreatedDate],itm.[ModifiedBy],itm.[ModifiedDate],itm.[Active],itm.[IsDeleted]
                            ...
                            Where 1 = 1 and itm.ApprovedBy is not null";// Start with a true condition for dynamic filtering
