...
namespace Inv.WebAPI.Controllers
{
    ...
    [ApiController]
    public class BrandController : GenericController<Brand, ApplicationDbContext, GetBrandDto, CreateBrandDto, UpdateBrandDto>
    {
        ...
        public BrandController(ApplicationDbContext context, ILogger<BrandController> logger, IMediator mediator, IMapper mapper, TheNumbersService theNumbersService)
        ...
        [AuthorizeMultiplePermissions(Permissions.Btn_SetViewBrand)]
        public override async Task<ActionResult<IEnumerable<GetBrandDto>>> GetAll()
        {
        ...
        [HttpGet("{id}")]
        [AuthorizeMultiplePermissions(Permissions.Btn_SetViewBrand, Permissions.Btn_SetViewBrand)]
        public override async Task<ActionResult<GetBrandDto>> Get(int id)
        {
        ...
        [AuthorizeMultiplePermissions(Permissions.Btn_SetSaveBrand)]
        public override async Task<ActionResult<GetBrandDto>> Create([FromBody] CreateBrandDto createDto, [FromQuery] string[]? uniquePropertyNames = null)
        {
            return await base.Create(createDto, new[] { "BrandName" });
        }
        ...
        /// <param name="uniquePropertyNames">Optional: Array of property names to check for uniqueness.</param>
        [HttpPut]
        [AuthorizeMultiplePermissions(Permissions.Btn_SetEditBrand)]
        public override async Task<ActionResult<GetBrandDto>> Update([FromBody] UpdateBrandDto updateDto, [FromQuery] string[]? uniquePropertyNames = null)
        {
            return await base.Update(updateDto, new[] { "BrandName" });
        }
         /// <summary>
        ...
        [HttpDelete("{id}")]
        [AuthorizeMultiplePermissions(Permissions.Btn_SetDeleteBrand)]
        ...
        [AuthorizeMultiplePermissions(Permissions.Btn_SetViewBrand)]
        public async Task<ActionResult<IEnumerable<GetBrandDto>>> GetByCustomCriteria([FromQuery] string criteria)
        {
            // Example of custom filtering logic
            var entities = await _context.Set<Brand>()
                .Where(a => a.BrandName.Contains(criteria)) // Filter by name containing the criteria
                .ToListAsync();

            var dtos = _mapper.Map<List<GetBrandDto>>(entities);
            return Ok(dtos);
        }
    }
}
